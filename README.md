...
窗口与上下文跟踪基于 Accessibility API（`AXObserver` 监听焦点窗口、焦点元素、标题与选区变化），并对 AX 调用设置短超时，避免目标应用繁忙时拖慢判定。语言判定使用终端上下文启发式、汉字占比启发式 + 限定中英文候选的 `NLLanguageRecognizer`。上下文关键词会优先匹配窗口标题，再匹配当前焦点元素及其父级区域暴露的短文本元数据；选区/光标变化不会单独触发关键词切换。智能自学习只保存组件短元数据摘要与中/英文状态，不保存输入正文、选中文本、光标附近文本或窗口正文。
...
- 智能判定会在焦点进入、窗口/标题变化以及少量强上下文变化时运行；若最近 0.5 秒内检测到键盘输入，选区/标题变化触发的切换会被跳过，避免打字过程中被迟到的自动切换打断。
