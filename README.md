...
- **智能上下文**（需辅助功能权限）：仅在目标应用通过 AX 暴露真实输入组件语义时启用。先应用密码框/终端等结构硬规则，再看输入框正文，随后才使用 placeholder、标题、描述、焦点区域、窗口可见内容和标题等语义兜底。终端 shell 默认英文，`codex` / `claude` / `opencode` 等 Agent 活跃区域默认中文；若应用只暴露空 `AXGroup`、弱文本或黑盒状态，会直接回退到窗口记忆/应用默认，不再硬猜具体输入框。用户手动纠正会被自学习记住，智能学习使用精确组件、组件类型、上下文三档指纹和投票置信度，避免一次误操作直接覆盖记录。**部分极复杂页面的 AX 树读取会占用主线程数百毫秒；连续超限后该应用自动暂停智能模式、改用窗口记忆，避免持续读取 UI 导致系统卡顿。**
- **智能上下文高级设置**：可在设置中维护终端 Bundle ID、多上下文宿主、Agent 关键词和聊天语义关键词，新工具出现时不需要等待代码更新。
...
窗口与上下文跟踪基于 Accessibility API（`AXObserver` 监听焦点窗口、焦点元素、标题与选区变化），并对 app 元素、焦点元素、父元素和遍历到的子元素都设置短超时，避免目标应用繁忙时拖慢判定。AX 回调会转投到 MainActor 处理，不依赖回调线程假设。语言判定使用终端上下文启发式、汉字占比启发式 + 限定中英文候选的 `NLLanguageRecognizer`；窗口/焦点区域文本采样有独立预算，并会在已有足够语言信号时提前停止。上下文关键词会优先匹配窗口标题，再匹配当前焦点元素及其父级区域暴露的短文本元数据；选区/光标变化不会单独触发关键词切换。智能自学习只保存组件短元数据摘要、投票计数和中/英文状态，不保存输入正文、选中文本、光标附近文本或窗口正文。

窗口 key 在无法读取系统窗口 ID 时，会使用标题的稳定 SHA256 摘要作为回退标识，不依赖 Swift 进程内随机化的 `hashValue`。智能学习记录使用版本化存储 envelope；旧版本记录会被归档保留，当前版本记录继续参与查找。
...
5. 如需支持新的终端、Electron 宿主或 Agent 工具，可在「智能」设置页追加对应 Bundle ID 或关键词。
...
- 智能自学习按精确组件、组件类型、上下文三档指纹逐级查找；同一应用里的对话输入、终端 shell、Agent 区域会分开学习。无任何标签时会退到组件类型或上下文档，但需要投票置信度达标才会生效。
- 智能判定会在焦点进入、窗口/标题变化以及少量强上下文变化时运行；若最近 0.5 秒内检测到键盘输入，选区/标题变化触发的切换会被跳过，避免打字过程中被迟到的自动切换打断。中文输入源组字期间出现短连续小写拼音时，也会跳过自动切英文。
- 密码输入框会强制使用英文输入源，不参与智能语义猜测。
